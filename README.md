...
```mermaid
graph TD
    root(["Is it bigger than<br>a breadbox?"])
    eggs(["Do you eat it<br>with eggs?"])
    mouse["a mouse"]
    spam["spam"]
    money(["Is it worth<br>a lot of money?"])
    trash["a bag of trash"]
    code(["Does it write code?"])
    house["a house"]
    coder["a programmer"]
    ...
    classDef quest fill:#add8e6,color:black;
    classDef answer fill:#ffffe0,color:black;
    class root,eggs,money,code quest;
    class mouse,spam,trash,house,coder answer
